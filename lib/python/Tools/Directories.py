...
def resolveFilename(scope, base="", path_prefix=None):
	...
	if base == "":
		...
		# This import must be here as this module finds the config file as part of the config initialisation.
		from Components.config import config
		skin = os.path.dirname(config.skin.primary_skin.value).decode()
		resolveList = [
		...
		if hasattr(config.skin, "display_skin"):
			skin = os.path.dirname(config.skin.display_skin.value).decode()
		else:
			skin = ""
		resolveList = [
		...
		from Components.config import config
		skin = os.path.dirname(config.skin.primary_skin.value).decode()
		display = os.path.dirname(config.skin.display_skin.value).decode() if hasattr(config.skin, "display_skin") else None
		resolveList = [
