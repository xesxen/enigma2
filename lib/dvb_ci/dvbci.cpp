...
PyObject *eDVBCIInterfaces::getDescrambleRules(int slotid)
{
	...
	if (!slot)
	{
		...
		PyErr_SetString(PyExc_IndexError, tmp);
		return 0;
	}
...
RESULT eDVBCIInterfaces::setDescrambleRules(int slotid, SWIG_PYOBJECT(ePyObject) obj )
{
	...
	while(size)
	{
		...
		const char *tmpstr = static_cast<const char*>(PyUnicode_DATA(refstr));
	...
	while(size)
	{
		...
		const char *tmpstr = static_cast<const char*>(PyUnicode_DATA(PyTuple_GET_ITEM(tuple, 0)));
