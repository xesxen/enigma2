...
void eListboxPythonStringContent::paint(gPainter &painter, eWindowStyle &style, const ePoint &offset, int selected)
{
	...
	if (validitem)
	{
		...
		{
			const char *string = PyUnicode_Check(item) ? static_cast<const char*>(PyUnicode_DATA(item)) : "<not-a-string>";
...
void eListboxPythonConfigContent::paint(gPainter &painter, eWindowStyle &style, const ePoint &offset, int selected)
{
	...
	if (m_list && cursorValid)
	{
		...
		if (PyTuple_Check(item))
		{
			...
			const char *string = (text && PyUnicode_Check(text)) ? static_cast<const char*>(PyUnicode_DATA(text)) : "<not-a-string>";
			...
			if (value && PyTuple_Check(value))
			{
				...
				const char *atype = (type && PyUnicode_Check(type)) ? static_cast<const char*>(PyUnicode_DATA(type)) : 0;
				...
				if (atype)
				{
					if (!strcmp(atype, "text"))
					{
						...
						const char *value = (pvalue && PyUnicode_Check(pvalue)) ? static_cast<const char*>(PyUnicode_DATA(pvalue)) : "<not-a-string>";
					...
					{
						ePyObject pvalue = PyTuple_GET_ITEM(value, 1);
						const char *text = (pvalue && PyUnicode_Check(pvalue)) ? static_cast<const char*>(PyUnicode_DATA(pvalue)) : "<not-a-string>";
					...
					else if (!strcmp(atype, "pixmap"))
					{
						...
						if (SwigFromPython(pixmap, ppixmap))
						{
							...
							const char *value = (ppixmap && PyUnicode_Check(ppixmap)) ? static_cast<const char*>(PyUnicode_DATA(ppixmap)) : "<not-a-string>";
							painter.setFont(fnt2);
							if (value_alignment_left)
								painter.renderText(eRect(ePoint(offset.x()-15, offset.y()), m_itemsize), value, gPainter::RT_HALIGN_LEFT | gPainter::RT_VALIGN_CENTER, border_color, border_size);
							else
	...
	if (m_list && cursorValid)
	{
		...
		for (int i = start; i < items_size; ++i)
		{
			...
			*/
				...
				const char *string = (PyUnicode_Check(pstring)) ? static_cast<const char*>(PyUnicode_DATA(pstring)) : "<not-a-string>";
